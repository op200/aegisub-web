...
free(dataPtr); free(namePtr);
if (openResult !== 0) throw new Error('open failed: ' + module_.UTF8ToString(module_._aegisub_core_last_error()));
console.log('open: OK');
...
console.log('  cue0:', JSON.stringify({ style: state.document.cues[0].style, text: state.document.cues[0].text, startMs: state.document.cues[0].startMs, endMs: state.document.cues[0].endMs }));
console.log('  style0:', JSON.stringify({ name: state.document.styles[0].name, fontSize: state.document.styles[0].fontSize }));
...
const applyCmds = JSON.stringify([{ type: 'updateCue', id: cue0.id, patch: { text: 'Edited text' } }]);
...
console.log('export: has Dialogue =', exported.includes('Dialogue:'), '| has Comment =', exported.includes('Comment:'));
...
// 8. 搜索（text 字段，忽略大小写）
const searchPtr = module_._aegisub_document_search(doc, module_.allocateUTF8(JSON.stringify({ find: 'hello', field: 'text', matchCase: false, useRegex: false })));
...
// 9. replace all
const repPtr = module_._aegisub_document_replace_all(doc, module_.allocateUTF8(JSON.stringify({ find: 'hello', replaceWith: 'hi', field: 'text', matchCase: false })));
...
if (stateRep.document.cues[0].text !== 'hi {\\i1}world{\\i0}') throw new Error('replace text mismatch');
