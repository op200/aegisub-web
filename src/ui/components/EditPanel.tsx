...
/** #RRGGBB → ASS &HBBGGRR&（dialog_colorpicker.cpp 写入格式） */
...
export function EditPanel({
  ...
  const colorTagRef = useRef<'c' | '2c' | '3c' | '4c'>('c')
  ...
  // edit/color/*：在光标处写入颜色标签（dialog_colorpicker.cpp）
  const openColorPicker = (tag: 'c' | '2c' | '3c' | '4c') => {
    colorTagRef.current = tag
    colorPickerRef.current?.click()
  }
  const applyColor = (hex: string) => {
    const editor = editorRef.current
    const tag = colorTagRef.current
    const insert = `\\${tag}${cssToAssColor(hex)}&`
    if (!editor) {
      commitText(`${draft.text}{${insert}}`, 'Edit text')
      return
    }
    const start = editor.selectionStart
    const end = editor.selectionEnd
    const nextText = `${draft.text.slice(0, start)}{${insert}}${draft.text.slice(end)}`
    commitText(nextText, 'Edit text')
    requestAnimationFrame(() =>
      editor.setSelectionRange(start + insert.length + 2, start + insert.length + 2),
    )
  }
      ...
      <div className="edit-row edit-row-top">
        ...
        <label className="edit-actor">
          ...
          <datalist id={`edit-actor-values-${cue.id}`}>
            {cue.actor && <option value={cue.actor} />}
          </datalist>
        </label>
        <label className="edit-effect">
          ...
          <datalist id={`edit-effect-values-${cue.id}`}>
            {cue.effect && <option value={cue.effect} />}
          </datalist>
        </label>
        <output
