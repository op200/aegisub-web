...
function detectBridge(): NativeBridge | null {
  if (window.chrome?.webview) {
    return { platform: 'webview2', postMessage: (message) => window.chrome!.webview!.postMessage(JSON.parse(message)) };
  }
  if (window.AegisubHost)
    return { platform: 'android-webview', postMessage: (message) => window.AegisubHost!.postMessage(message) };
  return null;
}
...
export class NativeHostAdapter implements HostAdapter {
  ...
  constructor(private readonly bridge: NativeBridge) {
    const receive = (data: unknown) => {
      const response = typeof data === 'string' ? (JSON.parse(data) as NativeResponse) : (data as NativeResponse);
    ...
    if (bridge.platform === 'webview2')
      ...
      window.addEventListener('aegisub-host-message', ((event: CustomEvent) => receive(event.detail)) as EventListener);
  }
